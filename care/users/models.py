...
class UserExternalIdentity(BaseModel):
    ...
    One row per `(provider_id, issuer, subject)`. The issuer is stored rather
    than read from live configuration on purpose: if a provider record is later
    pointed at a different issuer, enrolled identities stop resolving and login
    fails closed, instead of every enrolled account silently re-binding to
    whatever the new issuer says.

    Holding an identity grants nothing. It answers "which external subject is
    this", and CARE's existing roles and permissions answer everything else.
    """
    ...
    #: Null for a self-service link, where the principal is its own actor.
    #: SET_NULL because an audit trail that disappears with the administrator
    ...
    def __str__(self):
        return f"{self.provider_id}:{self.subject[:8]}… -> user {self.user_id}"
